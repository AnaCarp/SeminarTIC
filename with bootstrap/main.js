document.addEventListener('DOMContentLoaded', function () {
    ...
    function clickDelete() {
        if (!operation) {
            operand1 = operand1.slice(0, -1); // Șterge ultimul caracter din operand1
        } else {
            operand2 = operand2.slice(0, -1); // Șterge ultimul caracter din operand2
        }
        displayValue = operand1 + ' ' + operation + ' ' + operand2;
        updateDisplay();
    }
    ...
    buttons.forEach((button) => {
        button.addEventListener('click', () => {
            ...
            if (!isNaN(buttonValue) || buttonValue === '.') {
                ...
                clickClear();
            } else if (buttonValue === 'DEL') { // Adaugă această secțiune pentru butonul "DEL"
                clickDelete();
            }
        });
    });
